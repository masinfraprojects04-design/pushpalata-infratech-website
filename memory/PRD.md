...
- CERTIFICATE DOWNLOADS (Sep 10 2026): PDF split per certificate → /public/certificates/*.pdf + *.jpg (100dpi scan). Project.certificate.file/image; components/site/CertificateBlock.tsx on project detail (thumbnail → lightbox 'View Scan', 'Download PDF'). Old subcontract project cards now use REAL site photos as representative images (user has not supplied archive photos for 2014–2023 projects).
- ENQUIRY STATUS (Sep 10 2026): Enquiry.status new|contacted|closed (default new); PATCH /api/enquiries/{id}/status (admin). Admin inbox: status badge, 'Mark as' buttons per card, status filter chips with counts.
